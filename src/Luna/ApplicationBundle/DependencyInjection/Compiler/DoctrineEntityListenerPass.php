    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($ems as $name => $em) {
            $container->getDefinition(sprintf('doctrine.orm.%s_configuration', $name))
                ->addMethodCall('setEntityListenerResolver', [new Reference('nab.common.entity_listener_resolver')])
            ;
        }

        $definition = $container->getDefinition('nab.common.entity_listener_resolver');
